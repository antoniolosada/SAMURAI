  ...
  if (cCar == ':')
  {
    ...
    switch (cCar)
    {
      ...
      { // Movimiento temporizado
        ...
        {
          Secuencia[iPosSec].iTiempo = RecValor(Cadena, 3, '|', &iPosFin);
          RecCadena(Cadena, iPosFin+1, '|', &iPosFin); //Descripción
          Secuencia[iPosSec].BNumMov = RecValor(Cadena, iPosFin+1, '|', &iPosFin);
...
long RecValor(char *Cadena, int iPosIni, char cDelimitador, int *iFinPos)
{
  char cTmp[50] = "\0";
  char cCar;
  int i,j;
